...
import * as builder from "../../src/builders/runArgBuilder";
describe("Run Argument Builder", () => {
    ...
    it("should have noBuild if noBuild is passed", () => {
      assert.deepEqual(builder.default({noBuild: "yo" as any}), ["--no-build", "yo"]);
    });
    it("should have noRestore if noRestore is passed", () => {
      assert.deepEqual(builder.default({noRestore: "yo" as any}), ["--no-restore", "yo"]);
    });
    it("should have noDependencies if noDependencies is passed", () => {
      assert.deepEqual(builder.default({noDependencies: "yo" as any}), ["--no-dependencies", "yo"]);
    });
    it("should have force if force is passed", () => {
      assert.deepEqual(builder.default({force: "yo" as any}), ["--force", "yo"]);
    });
